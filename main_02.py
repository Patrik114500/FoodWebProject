...
class Simulation:
    def __init__(self, resource, prey_population, predator_population, 
                 grid_size, carrying_capacity, prey_var, predator_var):
        ...
        self.predator_var = predator_var
    ...
    def check_extinction(self):
        if len(self.prey_population) == 0:
            ...
            self.prey_population = [Agent(energy=5, metabolism=1, pos=(np.random.uniform(0, self.grid_size), np.random.uniform(0, self.grid_size))) for _ in range(self.prey_var.get())]
        ...
        if len(self.predator_population) == 0:
            ...
            self.predator_population = [Predator(energy=15, metabolism=0.8, prey_population=self.prey_population, pos=(np.random.uniform(0, self.grid_size), np.random.uniform(0, self.grid_size))) for _ in range(self.predator_var.get())]

####
global anim, simulation
...
def restart_simulation(grid_size, resource, prey_var, predator_var):
    global anim, simulation

    # Create new populations based on user-specified values
    prey_population = [Agent(energy=5, metabolism=1, pos=(np.random.uniform(0, grid_size), np.random.uniform(0, grid_size)))
                        for _ in range(prey_var.get())]
    predator_population = [Predator(energy=15, metabolism=0.8, prey_population=prey_population, pos=(np.random.uniform(0, grid_size), np.random.uniform(0, grid_size)))
                            for _ in range(predator_var.get())]
    # Reset the simulation instance
    simulation = Simulation(resource, prey_population, predator_population, grid_size, 2000, prey_var, predator_var)
    
    anim.event_source.stop()
    anim.event_source.start()

# Update function for animation
def update(prey_scatter, predator_scatter,
           prey_line, predator_line, ax2):
...
def animate(grid_size, resource, fig, speed_var, 
            prey_scatter, predator_scatter,
            prey_line, predator_line, ax2,
            prey_var, predator_var):
    ...
    simulation = Simulation(resource, prey_population, predator_population, grid_size, 2000, prey_var, predator_var)
    # Start animation
    anim = FuncAnimation(fig, update(prey_scatter, predator_scatter,
                                     prey_line, predator_line, ax2), frames=200, interval=speed_var.get(), repeat=False)
    plt.show()
...
def main():
    ...
    ttk.Button(control_frame, text="Run Simulation", command = animate(grid_size, resource, fig, speed_var, 
                                                                       prey_scatter, predator_scatter, 
                                                                       prey_line, predator_line, ax2,
                                                                       prey_var, predator_var)).grid(row=4, column=0, columnspan=3, pady=10)
    ...
    ttk.Button(control_frame, text="Restart", command=restart_simulation(grid_size, resource, prey_var, predator_var)).grid(row=3, column=2)
